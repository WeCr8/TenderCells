...
export default function SignUpForm({ 
  ...
  const { signUpWithEmail, signInWithProvider, error, clearError } = useAuth();
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setFormError(response.error?.message || 'Failed to sign up');
      }
    } catch (error) {
  ...
  const handleProviderSignUp = async (provider: AuthProvider) => {
    ...
    try {
      ...
      setFormError(error instanceof Error ? error.message : 'Failed to sign up with provider');
    }
  };
  ...
  return (
    <div className={`bg-white rounded-xl shadow-sm border border-gray-200 p-8 ${className}`}>
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600 mt-2">Join Chicken Tender to manage your farm</p>
      </div>
      ...
      {isTesting && (
        <div className="mt-6 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-sm text-blue-700">
            <strong>Testing Mode Active:</strong> Authentication is disabled. Click any sign-up method to proceed.
          </p>
        </div>
      )}
    </div>
