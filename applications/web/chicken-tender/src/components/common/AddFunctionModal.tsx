...
import type { MacroFunction, MacroParameter, MacroValidationResult } from '../../types/macros';

interface AddFunctionModalProps {
  isOpen: boolean;
  onClose: () => void;
  macro: MacroFunction | null;
  onExecute: (macroId: string, parameters: Record<string, any>) => Promise<void>;
  onSchedule?: (macroId: string, parameters: Record<string, any>, schedule: any) => Promise<void>;
  validateParameters?: (macroId: string, parameters: Record<string, any>) => Promise<MacroValidationResult>;
  className?: string;
}
...
export default function AddFunctionModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (validateParameters) {
        validationResult = await validateParameters(macro.id, parameters);
      ...
      if (showScheduleOptions && onSchedule) {
        await onSchedule(macro.id, parameters, schedule);
      } else {
        await onExecute(macro.id, parameters);
      }
      ...
      setErrors({
        _form: error instanceof Error ? error.message : 'An error occurred while executing the function'
      });
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`bg-white rounded-xl shadow-lg max-w-2xl w-full ${className}`}>
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 max-h-[70vh] overflow-y-auto">
          <div className="space-y-4">
            ...
            {macro.parameters.map((param) => {
              ...
              const shouldShow = !param.dependsOn || 
                (parameters[param.dependsOn] !== undefined && 
                 parameters[param.dependsOn] !== false && 
                 parameters[param.dependsOn] !== '');
